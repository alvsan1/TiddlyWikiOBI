...
let subclassOfURL = "http://www.w3.org/2000/01/rdf-schema#subClassOf";
let definitionTerm = "http://www.w3.org/2000/01/rdf-schema#label";
let labelTerm = "http://www.w3.org/2000/01/rdf-schema#label";
...
exports.startup = function(callback) {
	...
	vistas.forEach( function (nodeName) {
		...
		JSON.parse(nodeJson.concepts).forEach( function(ct){
			...
			if ( ct.property.value == subclassOfURL ) {
			...
			} else if ( ct.property.value == definitionTerm) {
				var textterm = "";
				JSON.parse(ct.oconcepts.value).forEach( function( oconcept ){
			...
			}else if ( ct.property.value == labelTerm ) {
				var textterm = "";
				JSON.parse(ct.oconcepts.value).forEach( function( oconcept ){
					textterm += "# " + oconcept + " <br>";
				});
